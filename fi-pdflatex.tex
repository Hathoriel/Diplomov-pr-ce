...
Zmíněné informace o uživateli jsou uloženy do objektu představujícího uživatele. 
...
\begin{tabular}{|l|l|}
\hline
\texttt{id (int)}                   & identifikátor perun id uživatele  \\ \hline
...
\texttt{sources (Array)}            & pole skupin, v kterých je uživatel členem\\ \hline
\texttt{sources.id (int)}           & identifikátor skupiny \\ \hline
\texttt{sources.name (String)}      & název skupiny \\ \hline
\texttt{sources.loa (int)}          & úroveň zabezpečení skupiny  \\ \hline
\texttt{sources.login (String)}     & identifikátor uživatele v dané skupině \\ \hline
