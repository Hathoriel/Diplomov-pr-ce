...
Autentizační a autorizační infrastruktura systému Adobe Connect je implementována v adresáři \texttt{/data/app/prod/aclogin-ng/} na který se budu v aktuální sekci odkazovat. Konfigurace představované implementace se nachází v podaresáři \texttt{config/} a samotná implementace autentizační a autorizační infrastruktury v podaresáři \texttt{lib/AcLogin/}. \par
...
Dále je v metodě \texttt{authorizeAction} provedena kontrola, zda je uživatel již přihlášen. Pokud tato skutečnost nastala, předá se řízení algoritmu metodě \texttt{responseAction} nacházející se v aktuální třídě \texttt{AuthorizeController}. Uvedená metoda zpracuje data udržované v kontextu o přihlášeném uživateli. Dále je vytvořen textový řetězcem identifikující sezení, tzv. \textbf{session} a \textbf{authorize code}. \textbf{Authorize code} je textový řetězec, který po autentizaci obdrží klient od autorizačního serveru. Zmíněný \textbf{authorize code} slouží k jednorázové výměně za tzv. \textbf{ID Token} nebo \textbf{Access Token}. Uvedeným žetonům se detailněji věnuji v rámci popisu technologií \hyperref[sec:oauth]{OAuth} a \hyperref[sec:oidc]{OpenID Connect}. S těmito údaji je uživatel přesměrovan na úvodní stránku přihlášeného uživatele portálu Shongo. Této metodě se věnuji dále v  \hyperref[sec:responseAction]{odstavci níže}. \par 
...
Po té se předá řízení algoritmu zpět do metody \texttt{dispatch} v třídě \texttt{Authorize}. V této metodě je dále z získaných uživatelských dat vytvořeno sezení a autorizační kód. Zmíněné sezení a autorizační kód jsou uloženy do interní databáze autentizačního serveru \texttt{shongo-authn.cesnet.cz}. Tyto uložené údaje jsou později využity při požadavku na získání Access Tokenu a uživatelských informací.
