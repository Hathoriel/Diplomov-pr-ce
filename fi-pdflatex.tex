...
Postup při přihlašování uživatele do systému Shongo je následující. Pokud se chce uživatel autentizovat, klikne na přihlašovací odkaz, který ho přesměruje na URL adresu \texttt{shongo-auth.cesnet.cz/authn/oic/authorize}. Tato URL adresa obsahuje další parametry \textbf{client\_id}, \textbf{redirect\_url}, \textbf{state}, \textbf{scope}, \textbf{response\_type} a \textbf{prompt} nezbytné pro dokončení autentizace. Zmíněná URL adresa je namapována na metodu \texttt{authorizeAction} třídy \texttt{AuthorizeController}, která se nachází v modulu \textbf{zf2-openid-connect-server-module} jakožto i následující představované třídy. V uvedené metodě \texttt{authorizeAction} začíná běh autentizačního algoritmu. \par 
V představené metodě \texttt{authorizeAction} je provedena kontrola, zda je uživatel již přihlášen. Pokud tato skutečnost nastala, předá se řízení algoritmu metodě \texttt{responseAction} nacházející se v aktuální třídě. Uvedená metoda zpracuje data udržované v kontextu o přihlášeném uživateli. Dále je uživatel přesměrován spolu s vytvořeným textovým řetězcem identifikující sezení, tzv. session na úvodní stránku přihlášeného uživatele portálu Shongo. Této metodě se věnuji detailněji v odstavci níže TODO odkaz. \par 
...
\end{itemize}
Dále jsou získány již dříve známé informace o křestním jménu, příjmení a emailové adrese uživatele. Uvedené informace o uživateli jsou přidány do již popisovaného objektu třídy \texttt{ShongoAuthn\textbackslash User\textbackslash User}. \par
Po té se předá řízení algoritmu zpět do metody \texttt{dispatch} v třídě \texttt{Authorize}. V této metodě je dále vytvořeno a uloženo sezení do interní databáze autentizačního serveru, které reprezentuje přihlášeného uživatele. 

TODO  schéma komunikace v při zpracování autentizačního požadavku do systému shongo. 
