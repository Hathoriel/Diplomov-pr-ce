...
\begin{itemize}
    \item \textbf{openid} představuje unikátní identifikátor uživatele v rámci Proxy IdP infrastruktury, tzv. einfra id,
...
\begin{itemize}
    ...
    \item \path{onGetUserIdByPrincipalName} představuje metodu určenou k získání uživatelského \textbf{perun id} pomocí \textbf{eduPersonPrincipalName},
...
Jak již bylo poznamenáno, první z metod \path{onGetUserDataByUserId} je v systému \path{meetings.cesnet.cz} využívána k získání informací\break o konkrétním uživateli pomocí jeho identifikátoru. Ve stávající implementaci se jako referenční identifikátor využívalo pouze \textbf{perun id}. V nové implementaci se uživatelské informace vyhledávají i pomocí identifikátoru \textbf{einfra id}. Funkcionalitu vyhledávání uživatele prostřednictvím \textbf{perun id} pomocí webové služby Perun jsem ponechal, protože je využívána při přidávání dalších účastníků ve vytvářeném rezervačním požadavku. 
