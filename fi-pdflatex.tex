...
\par 
Je důležité připomenou, ústřední prvky stávající implementace. V stávající implementaci jsou uživatelské atributy po autentizaci prostřednictvím systému Shibboleth mapovány na objekt představujícího uživatele. Jelikož je systém Shibboleth nahrazen za OpenID Connect poskytovatele, klíčovým prvkem implementace je přemapování zmíněných uživatelských atributů. Jde o atributy, které po autentizaci u OpenID Connect poskytovatele zprostředkuje nainstalovaný modul Apache \textbf{mod\_auth\_openidc}. 

První změnou byla editace konfiguračního souboru \texttt{config/connect.ini}. Tento soubor zajišťuje mapování získaných uživatelských atributů z proměnných prostředí Apache serveru po autentizaci. Následující výňatek konfiguračního souboru popisuje nové mapování uživatelských atributů do proměnných autentizačního algoritmu. 
...
\end{lstlisting}
První řádek výňatku mapuje unikátní identifikátor einfra id, druhý řádek seznam obsahující \textbf{eduPersonPrincipalName}, další řádek emailovou adresu, předposlední řádek jméno a poslední řádek příjmení uživatele. Uvedené atributy jsou nastaveny objektu představujícího uživatele s kterým se dál v algoritmu pracuje. \par 
TODO rozepsat a propojit predchozi odstavec s nasledujicim
...
\par 
V neposlední řadě, je důležité poznamenat, že nová implementace autorizační a autentizační vrstvy systému Adobe Connect byla přesunuta do adresáře \texttt{/app/oidc/}. Dále je důležité zmínit, že chráněná URL adresa definovaná v \hyperref[ac-location]{konfiguračním souboru} Apache je mapována právě na uvedený adresář \texttt{/app/oidc/}.
